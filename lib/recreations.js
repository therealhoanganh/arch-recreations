...
function seriesNoteFields(series, links, opts = {}) {
  const tags = (opts.tags || []).map((t) => String(t).replace(/^#+/, '').trim()).filter(Boolean);
  return {
    seasons: series.seasons.length || undefined,
...
// Art first, then the seasons as wikilinks -- the season notes are where the
// episodes live.
function seriesNoteBody(posterLink, backdropLinks, seasonLinks) {
  const art = [posterLink, ...backdropLinks].filter(Boolean).map((l) => `!${l}`);
  const parts = [];
  if (art.length) parts.push(art.join('\n'));
  if (seasonLinks.length) parts.push(seasonLinks.map((l) => `- ${l}`).join('\n'));
...
const EPISODES_OPEN = '%% episodes %%';
const EPISODES_CLOSE = '%% /episodes %%';

// One line per episode: an Open link once the file is there, plain text until
// then. The list sits between Obsidian comment markers, invisible in reading
// view, so a later check can replace just the list and leave anything written
// around it alone.
function episodeList(episodes, files, player) {
  ...
  return `${EPISODES_OPEN}\n${lines.join('\n')}\n${EPISODES_CLOSE}`;
}

function seasonNoteBody(posterLink, episodes, files, player) {
  const parts = [];
  if (posterLink) parts.push(`!${posterLink}`);
  parts.push(episodeList(episodes, files, player));
  return parts.join('\n\n') + '\n';
}

// Swaps the episode block in an existing body; appends one when the markers
// are missing (a note written before they existed, or a hand-made one).
function replaceEpisodeList(body, episodes, files, player) {
  const block = episodeList(episodes, files, player);
  const start = body.indexOf(EPISODES_OPEN);
  const end = body.indexOf(EPISODES_CLOSE);
  if (start >= 0 && end > start) return body.slice(0, start) + block + body.slice(end + EPISODES_CLOSE.length);
