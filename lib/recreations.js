...
const OWN_KEYS = new Set([
  'plot', 'duration', 'year', 'URL', 'poster', 'banner', 'genres', 'director', 'writer', 'actors',
...
function seriesNoteFields(series, links, opts = {}) {
  ...
  return {
    ...
    plot: series.overview || undefined,
...
function seasonNoteFields(season, seriesLink, links, opts = {}) {
  const tags = (opts.tags || []).map((t) => String(t).replace(/^#+/, '').trim()).filter(Boolean);
  return {
    plot: season.overview || undefined,
...
// Why a 4K release is unfit, or '' if it is fine. Titles on The Pirate Bay
// are cut off mid-word ("DV HDR H 2"), so the codec cannot be required by
// name; what is ruled out is what the name does say:
// - not 2160p by Radarr/Sonarr's own parse;
// - AV1: the 2020 Intel MacBook this was built for has no hardware decoder
//   for it, and 4K AV1 stutters;
// - remux: tens of gigabytes for a picture mostly watched on a 13" screen;
// - Dolby Vision with no HDR layer (profile 5): VLC shows it purple and
//   green. "DV HDR" carries an HDR10 fallback and plays correctly.
function unfit4K(r) {
  const title = String(r.title || '');
  const res = r.quality && r.quality.quality && r.quality.quality.resolution;
  if (res && res !== 2160) return `${res}p`;
  if (/\bAV1\b/i.test(title)) return 'AV1';
  if (/remux/i.test(title)) return 'remux';
  if (/\b(DV|DoVi|Dolby[ .]?Vision)\b/i.test(title) && !/HDR/i.test(title)) return 'Dolby Vision without HDR';
  return '';
}

// The 4K pick. Size matters first, but a release with too few seeders takes
// forever, so: among fit releases with at least `enough` seeders, the
// smallest wins (an HDR one before one without). Nothing that well seeded
// falls back to the ordinary cascade over the fit releases. A season takes
// packs only, when any fit pack exists -- one torrent, one release group --
// since grabbing a single-episode release fetches just that episode.
// Too-small files are Radarr/Sonarr's to reject, through their quality
// definitions' minimum size per minute.
function pick4KRelease(releases, tiers, enough, opts = {}) {
  let fit = (releases || []).filter((r) => !r.rejected && !unfit4K(r));
  if (opts.season) {
    const packs = fit.filter((r) => r.fullSeason);
    if (packs.length) fit = packs;
  }
  const seeded = fit.filter((r) => (r.seeders || 0) >= enough);
  if (seeded.length) {
    seeded.sort(
      (a, b) =>
        Number(/HDR/i.test(b.title || '')) - Number(/HDR/i.test(a.title || '')) ||
        (a.size || 0) - (b.size || 0)
    );
    return { release: seeded[0], floor: enough, smallest: true };
  }
  const cascade = pickRelease(fit, tiers);
  return cascade && { ...cascade, smallest: false };
}
...
// mounted drive. The first that exists wins, with symlinks resolved: on the
// PC `/Volumes/4T-HDD` is a link to `/run/media/...`, and a snap player (VLC
// there) has no `/Volumes` inside its sandbox, so it must be handed the real
// path.
function locateFile(absPath, platform = process.platform) {
  const found = findFile(absPath, platform);
...
function findFile(absPath, platform) {
